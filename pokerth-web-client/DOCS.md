...
This app runs the official [PokerTH web client](https://github.com/narmod/pokerth-web-client): a browser PWA plus its Node.js WebSocket-to-TCP proxy, the same stack that powers https://webclient.pokerth.net.

The install is **docker-git**: the Docker image only ships the runtime, and the web client is cloned from GitHub into `/data/app` on first start. It survives app updates, and it can update itself straight from its admin page — no image rebuild needed.
...
1. Set `admin_token` in the app's **Configuration** tab (any long random string).
2. Restart the app.
...
For the full update's restart to bring the app back automatically, enable the app's **Watchdog** toggle in Home Assistant (the proxy stops itself and the Watchdog restarts it). Alternatively set `auto_update: true` and restart the app from Home Assistant.
...
| `auto_update` | `false` | Sync the checkout to `git_ref` on every app start. |
...
The host port can be changed in the app's **Network** section (default 8080). The sidebar panel follows the remapped port automatically.

Note: the app is reachable on your local network only — Nabu Casa / the HA cloud URL tunnels the Home Assistant interface, not the app's port. Away from home, play on https://webclient.pokerth.net (same client, same servers).
...
Both survive app updates and rebuilds. First start needs internet access to clone the client (~a minute on a Raspberry Pi, npm install included).
